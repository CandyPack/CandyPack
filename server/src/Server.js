class Server {
  ...
  async services() {
    return new Promise((resolve, reject) => {
      ...
      let running = 0;
      for (const service of services)
        if (service.active && service.status == 'running') running++;
  ...
  restart() {
    return new Promise((resolve) => {
      __('Restarting CandyPack Server...').then((text) => {
        Candy.Cli.log(text);
        this.stop();
  ...
  watchdog() {
    __('Starting CandyPack Server...').then((text) => {
      console.log(text);
      let child = Candy.ext.childProcess.spawn(
        'node',
        [__dirname + '/../watchdog.js', 'start'],
        { detached: true },
      );
