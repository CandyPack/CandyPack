class Subdomain {
  create(subdomain) {
    ...
    if (Candy.config.websites[domain.join('.')])
      return Candy.Api.result(false, __('Domain %s already exists.', domain.join('.')))
    ...
    if (typeof domain == 'object')
      return Candy.Api.result(false, __('Domain %s not found.', domain.join('.')))
    ...
    return Candy.Api.result(
      true,
      __('Subdomain %s created successfully for domain %s.', fulldomain, domain)
    )
  }
  ...
  list(domain) {
    ...
    let subdomains = Candy.config.websites[domain].subdomain.map(subdomain => {
