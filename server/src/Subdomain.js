class Subdomain {
  create(subdomain) {
    ...
    if (subdomain.length < 3)
      return Candy.Api.result(false, __('Invalid subdomain name.'));
    if (Candy.config.websites[domain.join('.')])
      return Candy.Api.result(
        false,
        __('Domain %s already exists.', domain.join('.')),
      );
    ...
    if (typeof domain == 'object')
      return Candy.Api.result(
        false,
        __('Domain %s not found.', domain.join('.')),
      );
    subdomain = subdomain
      .join('.')
      .substr(0, subdomain.join('.').length - domain.length - 1);
    let fulldomain = [subdomain, domain].join('.');
    if (Candy.config.websites[domain].subdomain.includes(subdomain))
      return Candy.Api.result(
        false,
        __('Subdomain %s already exists.', fulldomain),
      );
    Candy.DNS.record(
      { name: fulldomain, type: 'A' },
    ...
    return Candy.Api.result(
      true,
      __(
        'Subdomain %s created successfully for domain %s.',
        fulldomain,
        domain,
      ),
    );
  ...
  list(domain) {
    ...
    let subdomains = Candy.config.websites[domain].subdomain.map(
      (subdomain) => {
