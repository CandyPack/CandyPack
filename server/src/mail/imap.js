class Connection {
          ...
          this.#options.onAuth(
            ...
            (err, result) => {
              if (err) {
                ...
                this.#write(
                  `${this.#request.id} OK Authentication successful\r\n`,
                );
                log(
                  'mail',
                  'imap',
                  'Authentication successful for: ' + auth[1],
                );
                this.#auth = auth[1];
              }
            },
  ...
  #data(data) {
    ...
    this.#request.action = data
      .filter((item) => Object.keys(this.#actions).includes(item.toUpperCase()))
      .join(' ');
    ...
    if (data.includes('UID') && data.indexOf('UID') < index) {
      ...
      if (!data.includes('UID') || !data.includes('(UID'))
        data.splice(0, 0, 'UID');
    } else if (index == 0) {
  ...
  #examine() {
    if (!this.#auth)
      return this.#write(`${this.#request.id} NO Authentication required\r\n`);
    ...
    this.#options.onSelect(this.#auth, this.#options, (data) => {
      ...
      if (data.exists !== undefined)
        this.#write('* ' + data.exists + ' EXISTS\r\n');
      this.#write('* ' + (data.recent ?? data.exists) + ' RECENT\r\n');
      if (data.unseen !== undefined)
        this.#write(
          '* OK [UNSEEN ' + data.unseen + '] Message ' + data.unseen ??
            0 + ' is first unseen\r\n',
        );
      if (data.uidvalidity !== undefined)
        this.#write(
          '* OK [UIDVALIDITY ' + data.uidvalidity + '] UIDs valid\r\n',
        );
      if (data.uidnext !== undefined)
        this.#write(
          '* OK [UIDNEXT ' + data.uidnext + '] Predicted next UID\r\n',
        );
  ...
  async #fetch() {
    if (!this.#auth)
      return this.#write(`${this.#request.id} NO Authentication required\r\n`);
    if (!this.#box)
      return this.#write(`${this.#request.id} NO Mailbox required\r\n`);
        ...
        this.#options.onFetch(
          {
            ...
            mailbox: this.#box,
            limit:
              id == 'ALL' ? null : id.includes(':') ? id.split(':') : [id, id],
          },
          this.#commands,
          (data) => {
            if (data === false)
  ...
  #login() {
    if (this.#options.onAuth && typeof this.#options.onAuth == 'function') {
      if (this.#commands[2].startsWith('"') && this.#commands[2].endsWith('"'))
        this.#commands[2] = this.#commands[2].substr(
          1,
          this.#commands[2].length - 2,
        );
      if (this.#commands[3].startsWith('"') && this.#commands[3].endsWith('"'))
        this.#commands[3] = this.#commands[3].substr(
          1,
          this.#commands[3].length - 2,
        );
      this.#options.onAuth(
        {
  ...
  #prepare(requests, data) {
    ...
    for (let request of requests) {
      if (typeof data.headerLines === 'string')
        data.headerLines = JSON.parse(data.headerLines);
      let boundary = data.headerLines.find(
        (line) => line.key.toLowerCase() == 'content-type',
      );
      if (boundary)
        boundary = boundary.line.replace(/"/g, '').split('boundary=')[1];
      if (!boundary) boundary = 'boundary' + data.id;
      switch (request.value) {
        case 'BODY': {
          let body = { keys: '', header: '', content: '' };
          for (let obj of request.fields.length
            ? request.fields
            : [{ value: 'HEADER' }, { value: 'TEXT' }]) {
            let fields = obj.fields
              ? obj.fields.map((field) => field.value.toLowerCase())
              : [];
            if (request.fields.length)
              body.keys += obj.value + (obj.peek ? '.' + obj.peek : '');
            ...
            if (obj.value == 'HEADER') {
              for (let line of data.headerLines) {
                ...
                if (obj.peek)
                  ...
                  else if (obj.peek == 'FIELDS.NOT')
                    include = !fields.includes(line.key);
                if (include) {
              ...
              if (data.html.length > 1 || data.attachments.length) {
                ...
                for (let attachment of data.attachments) {
                  ...
                  body.content +=
                    ...
                    attachment.filename +
                    '"\r\n\r\n';
                  body.content += Buffer.from(attachment.content.data).toString(
                    'base64',
                  );
                }
              ...
              if (
                obj.value === 1 ||
                (obj.value === 2 && !data.attachments.length)
              ) {
                if (obj.peek === 2 || obj.value === 2)
                  body.content += data.html;
                else body.content += data.text;
              } else if (obj.value > 1 && data.attachments[obj.value - 2])
                body.content +=
                  Buffer.from(
                    data.attachments[obj.value - 2].content.data,
                  ).toString('base64') + '\r\n';
            }
          }
        ...
        case 'BODYSTRUCTURE': {
          ...
          if (data.text && data.text.length && data.html && data.html.length)
            structure +=
              ' "ALTERNATIVE" ("BOUNDARY" "' + boundary + '_alt") NIL NIL NIL';
          if (data.text && data.text.length && data.html && data.html.length)
            structure += ')';
          for (let attachment of data.attachments)
            structure +=
              ...
              '")) NIL NIL)';
          if (data.attachments.length)
            structure += ' "MIXED" ("BOUNDARY" "' + boundary + '") NIL NIL NIL';
  ...
  #select() {
    ...
    this.#options.onSelect(
      { address: this.#auth, mailbox: this.#box },
      this.#options,
      (data) => {
        this.#write(
          '* FLAGS (\\Answered \\Flagged \\Deleted \\Seen \\Draft)\r\n',
        ...
        this.#write(
          '* OK [UIDNEXT ' + (data.uidnext ?? 1) + '] Predicted next UID\r\n',
        );
  ...
  #status() {
    ...
    let fields = this.#commands
      .slice(3)
      .map((field) => field.toUpperCase().replace('(', '').replace(')', ''));
    if (fields.length === 0)
      fields = ['MESSAGES', 'RECENT', 'UIDNEXT', 'UIDVALIDITY', 'UNSEEN'];
    this.#options.onSelect(
      { address: this.#auth, mailbox: mailbox },
      this.#options,
      (data) => {
  ...
  #store() {
    ...
    for (let field of this.#request.requests) {
      switch (field.value) {
        case '+FLAGS': {
          let flags = field.fields.map((flag) =>
            flag.value.replace('\\', '').toLowerCase(),
          );
          this.#options.onStore(
            { address: this.#auth, uids: uids, flags: flags },
            this.#options,
            (data) => {
