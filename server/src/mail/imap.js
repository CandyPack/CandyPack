class Connection {
  ...
  #authenticate() {
    ...
    log(
      'mail',
      'imap',
      'Authenticate request from: ' + this.#socket.remoteAddress.replace('::ffff:', '')
    )
    this.#wait = true
  ...
  #examine() {
    ...
    this.#options.onSelect(this.#auth, this.#options, data => {
      ...
      if (data.unseen !== undefined)
        this.#write(
          '* OK [UNSEEN ' + data.unseen + '] Message ' + data.unseen ?? 0 + ' is first unseen\r\n'
        )
    ...
    for (let request of requests) {
      ...
      switch (request.value) {
        case 'BODY':
          ...
          for (let obj of request.fields.length
            ...
            if (obj.value == 'HEADER') {
              ...
              if (obj.value === 1 || (obj.value === 2 && !data.attachments.length)) {
                ...
                body.content +=
                  Buffer.from(data.attachments[obj.value - 2].content.data).toString('base64') +
                  '\r\n'
            }
          }
          ...
          this.#write(
            'BODY[' + body.keys + '] {' + Buffer.byteLength(body.content, 'utf8') + '}\r\n'
          )
    ...
    this.#options.onSelect({address: this.#auth, mailbox: mailbox}, this.#options, data => {
      ...
      for (let field of fields)
        if (data[field.toLowerCase()] !== undefined)
          this.#write(field.toUpperCase() + ' ' + (data[field.toLowerCase()] ?? 0) + ' ')
      this.#write(')\r\n')
