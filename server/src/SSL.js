class SSL {
  ...
  async check() {
    ...
    for (const domain of Object.keys(Candy.config.websites)) {
      ...
      if (
        !Candy.config.websites[domain].cert?.ssl ||
        Date.now() + 1000 * 60 * 60 * 24 * 30 >
          Candy.config.websites[domain].cert.ssl.expiry
      )
  ...
  renew(domain) {
    ...
    return Candy.Api.result(
      true,
      __('SSL certificate for domain %s renewed successfully.', domain),
    );
  }
  ...
  #self() {
    ...
    const pems = Candy.ext.selfsigned.generate(attrs, {
      days: 365,
      keySize: 2048,
    });
    if (
      !Candy.ext.fs.existsSync(Candy.ext.os.homedir() + '/.candypack/cert/ssl')
    )
      Candy.ext.fs.mkdirSync(Candy.ext.os.homedir() + '/.candypack/cert/ssl', {
        recursive: true,
      });
    let key_file =
      Candy.ext.os.homedir() + '/.candypack/cert/ssl/candypack.key';
    let crt_file =
      Candy.ext.os.homedir() + '/.candypack/cert/ssl/candypack.crt';
  ...
  async #ssl(domain) {
    ...
    for (const subdomain of Candy.config.websites[domain].subdomain ?? [])
    ...
    try {
      ...
      if (this.#checked[domain].error < 5)
        this.#checked[domain].error = this.#checked[domain]
          ? this.#checked[domain].error + 1
          : 1;
      this.#checked[domain].interval =
        this.#checked[domain].error * 1000 * 60 * 5 + Date.now();
      console.log(e);
      return;
    }
    if (!cert) return;
    ...
    Candy.ext.fs.writeFileSync(
      Candy.ext.os.homedir() + '/.candypack/cert/ssl/' + domain + '.key',
      key,
    );
    Candy.ext.fs.writeFileSync(
      Candy.ext.os.homedir() + '/.candypack/cert/ssl/' + domain + '.crt',
      cert,
    );
