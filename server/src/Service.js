...
async function run(id) {
  ...
  if (
    (service.status == 'errored' || service.status == 'stopped') &&
    Date.now() - service.updated < error_counts[id] * 1000
  ) {
    active[id] = false;
    return;
  }
  ...
  child.stdout.on('data', function (data) {
    ...
    logs[id] +=
      ...
      data
        ...
        .join('\n[LOG][' + Date.now() + '] ') +
      '\n';
    if (logs[id].length > 1000000)
      logs[id] = logs[id].substr(logs[id].length - 1000000);
  });
  child.stderr.on('data', function (data) {
    ...
    if (errs[id].length > 1000000)
      errs[id] = errs[id].substr(errs[id].length - 1000000);
    set(id, {
      status: 'errored',
      updated: Date.now(),
    });
      ...
      if (file && file.length > 0) {
        ...
        if (Candy.ext.fs.existsSync(file)) {
          if (!get(file)) add(file);
            ...
            return resolve(
              Candy.Api.result(true, __('Service %s already exists.', file)),
            );
        } else {
          return resolve(
            Candy.Api.result(false, __('Service file %s not found.', file)),
          );
        }
      } else {
        return resolve(
          Candy.Api.result(false, __('Service file not specified.')),
        );
      }
    });
  },
