...
class DNS {
  ...
  #request(request, response) {
    ...
    while (!Candy.core('Config').config.websites[domain] && domain.includes('.')) domain = domain.split('.').slice(1).join('.')
    if (!Candy.core('Config').config.websites[domain]) return response.send()
    for (const record of Candy.core('Config').config.websites[domain].DNS.A ?? []) {
    ...
    for (const record of Candy.core('Config').config.websites[domain].DNS.CNAME ?? []) {
      if (record.name != response.question[0].name) continue
      response.answer.push(
        dns.CNAME({
          name: record.name,
    ...
    for (const record of Candy.core('Config').config.websites[domain].DNS.MX ?? []) {
      if (record.name != response.question[0].name) continue
      response.answer.push(
        dns.MX({
          name: record.name,
    ...
    for (const record of Candy.core('Config').config.websites[domain].DNS.NS ?? []) {
      if (record.name != response.question[0].name) continue
    ...
    for (const record of Candy.core('Config').config.websites[domain].DNS.TXT ?? []) {
    ...
    for (const record of Candy.core('Config').config.websites[domain].DNS.SOA ?? []) {
      response.header.aa = 1
      response.authority.push(
        dns.SOA({
          name: record.name,
  ...
  record(...args) {
    ...
    for (let obj of args) {
      ...
      if (obj.unique && obj.unique === false)
        Candy.core('Config').config.websites[domain].DNS[type].filter(record => record.name != obj.name)
