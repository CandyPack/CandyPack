class DNS {
  ...
  delete(...args) {
    ...
    for (let obj of args) {
      let domain = obj.name;
      while (!Candy.config.websites[domain] && domain.includes('.'))
      ...
      if (
        ...
        !Candy.config.websites[domain].DNS[type]
      )
        continue;
      Candy.config.websites[domain].DNS[type] = Candy.config.websites[
        domain
      ].DNS[type].filter(
        (record) =>
          record.name != obj.name && (record.value != obj.value || !obj.value),
      );
    }
  }
  ...
  #publish() {
    if (this.#loaded || !Object.keys(Candy.config.websites ?? []).length)
      return;
    this.#loaded = true;
    ...
    this.#udp.on('error', (err, buff, req, res) =>
      console.error('DNS', err.stack),
    );
    this.#tcp.on('error', (err, buff, req, res) =>
      console.error('DNS', err.stack),
    );
    this.#udp.serve(53);
    this.#tcp.serve(53);
  ...
  record(...args) {
    ...
    for (let obj of args) {
      ...
      delete obj.type;
      if (!this.#types.includes(type)) continue;
      if (!Candy.config.websites[domain].DNS)
        Candy.config.websites[domain].DNS = {};
      if (!Candy.config.websites[domain].DNS[type])
        Candy.config.websites[domain].DNS[type] = [];
      if (obj.unique && obj.unique === false)
        Candy.config.websites[domain].DNS[type].filter(
    ...
    for (let domain of domains)
      Candy.config.websites[domain].DNS.SOA = [
        {
          name: domain,
          value:
            'ns1.' +
