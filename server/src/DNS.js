class DNS {
  ...
  delete(...args) {
    ...
    for (let obj of args) {
      ...
      Candy.config.websites[domain].DNS[type] = Candy.config.websites[domain].DNS[type].filter(
  ...
  record(...args) {
    ...
    for (let obj of args) {
      ...
      if (obj.unique && obj.unique === false)
        Candy.config.websites[domain].DNS[type].filter(record => record.name != obj.name)
