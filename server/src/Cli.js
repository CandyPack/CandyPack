class Cli {
      ...
      sub: {
        ...
        delete: {
          ...
          action: async () =>
            await this.#call({
              action: 'mail.delete',
              data: [await this.question(await __('Enter the e-mail address: '))]
            })
        },
        list: {
        ...
        create: {
          ...
          action: async () =>
            await this.#call({
              ...
              data: [
                await this.question(await __('Enter the subdomain name (subdomain.example.com): '))
              ]
            })
        },
        list: {
        ...
        create: {
          ...
          action: async () => {
            ...
            await this.#call({
              ...
              data: [
                domain,
                (await this.question(await __('Enter the path to the website (%s): ', path))) ??
                  path
              ]
            })
          }
  ...
  #call(command) {
    if (!command) return
    Candy.ext.axios
      .post('http://127.0.0.1:1453', command, {headers: {Authorization: Candy.config.api.auth}})
      .then(response => {
  ...
  async debug() {
    process.stdout.write(
      process.platform === 'win32' ? `title CandyPack Debug\n` : `\x1b]2;CandyPack Debug\x1b\x5c`
    )
    await this.#debug()
  ...
  async #help(commands) {
    ...
    if (typeof commands == 'string') {
      ...
      if (!obj[command])
        return log(
          __(`'%s' is not a valid command.`, this.#color(`candy ${commands.join(' ')}`, 'yellow'))
        )
      obj = obj[command]
  ...
  init() {
    ...
    if (!this.#commands[command])
      return log(
        __(`'%s' is not a valid command.`, this.#color(`candy ${cmds.join(' ')}`, 'yellow'))
      )
    ...
    this.logs.content = log
      ...
      .map(line => {
        if ('[LOG]' == line.substr(0, 5)) {
          ...
          line =
            this.#color('[' + this.#formatDate(new Date(date)) + ']', 'green', 'bold') +
            line.substr(15)
        } else if ('[ERR]' == line.substr(0, 5)) {
          line = line.substr(5)
          let date = parseInt(line.substr(1, 13))
          line =
            this.#color('[' + this.#formatDate(new Date(date)) + ']', 'red', 'bold') +
            line.substr(15)
        }
        return line
      })
  ...
  async #loadModuleLogs() {
    ...
    if (Candy.ext.fs.existsSync(file)) {
      mtime = Candy.ext.fs.statSync(file).mtime
      if (
        JSON.stringify(this.#watch) === JSON.stringify(this.logs.watched) &&
        mtime == this.logs.mtime
      ) {
        this.logging = false
        return
      }
  ...
  async monitor() {
    ...
    this.rl.input.on('keypress', (key, data) => {
      ...
      if (data.name == 'up') if (this.selected > 0) this.selected--
      if (data.name == 'down')
        if (this.selected + 1 < this.domains.length + this.services.length) this.selected++
      process.stdout.clearLine(0)
      process.stdout.cursorTo(0)
      this.#monitor()
    })
          ...
          for (let iterator of ['Status', 'Uptime', 'Websites', 'Services', 'Auth']) {
            ...
            if (i) {
              ...
              switch (iterator) {
                ...
                case 'Uptime':
                  if (status.online)
                    log(title + space + ' : ' + '\x1b[32m ' + status.uptime + '\x1b[0m')
                  break
                case 'Websites':
                  if (status.online)
                    log(title + space + ' : ' + '\x1b[32m ' + status.websites + '\x1b[0m')
                  break
                case 'Services':
                  if (status.online)
                    log(title + space + ' : ' + '\x1b[32m ' + status.services + '\x1b[0m')
                  break
                case 'Auth':
                  log(
                    title +
