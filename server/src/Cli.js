class Cli {
      ...
      sub: {
        ...
        list: {
          ...
          action: async () =>
            await this.#call({
              action: 'mail.list',
              data: [await this.question(await __('Enter the domain name: '))],
            }),
        },
        password: {
          ...
          action: async () =>
            await this.#call({
              ...
              data: [
                await this.question(
                  await __(
                    'Enter the subdomain name (subdomain.example.com): ',
                  ),
                ),
              ],
            }),
      ...
      sub: {
        create: {
          ...
          action: async () => {
            ...
            let path =
              Candy.ext.path.resolve().replace(/\\/g, '/') + '/' + domain + '/';
            await this.#call({
              action: 'web.create',
              data: [
                domain,
                (await this.question(
                  await __('Enter the path to the website (%s): ', path),
                )) ?? path,
              ],
            });
          },
  ...
  #call(command) {
    if (!command) return;
    Candy.ext.axios
      .post('http://127.0.0.1:1453', command, {
        headers: { Authorization: Candy.config.api.auth },
      })
      .then((response) => {
  ...
  #color(text, color, ...args) {
    ...
    if (this.#colors[color])
      output = '\x1b[' + this.#colors[color] + 'm' + output + '\x1b[0m';
    for (const arg of args) {
      if (this.#backgrounds[arg])
        output = '\x1b[' + this.#backgrounds[arg] + 'm' + output + '\x1b[0m';
  ...
  #debug() {
    ...
    result += ' ' + this.#color(title) + ' ';
    result += this.#color(
      '─'.repeat(this.width - c1 - title.length - 7),
      'gray',
    );
    result += this.#color('┐ \n', 'gray');
    for (let i = 0; i < this.height - 6; i++) {
      ...
      if (this.logs.selected == this.selected) {
        result += this.#spacing(
          this.logs.content[i] ? this.logs.content[i] : ' ',
          this.width - c1,
        );
      } else {
      ...
      if (!obj[command])
        return log(
          __(
            ...
            this.#color(`candy ${commands.join(' ')}`, 'yellow'),
          ),
        );
      obj = obj[command];
  ...
  #icon(status, selected) {
    if (status == 'running')
      return this.#color(' \u25B6 ', 'green', selected ? 'white' : null);
    if (status == 'stopped')
      return this.#color(' \u23F8 ', 'yellow', selected ? 'white' : null);
    if (status == 'errored')
      return this.#color(' ! ', 'red', selected ? 'white' : null);
    return '   ';
  }
  ...
  init() {
    ...
    if (!this.#commands[command])
      return log(
        __(
          `'%s' is not a valid command.`,
          this.#color(`candy ${cmds.join(' ')}`, 'yellow'),
        ),
      );
  ...
  async #load() {
    ...
    if (this.selected < this.domains.length) {
      ...
      file =
        Candy.ext.os.homedir() +
        '/.candypack/logs/' +
        this.services[this.selected - this.domains.length].name +
        '.log';
    } else {
      this.logging = false;
      return;
    ...
    if (Candy.ext.fs.existsSync(file)) {
      ...
      if (this.selected == this.logs.selected && mtime == this.logs.mtime)
        return;
  ...
  #monitor() {
    ...
    for (let i = 0; i < this.height - 4; i++) {
      if (this.domains[i]) {
        result += this.#color(' │', 'gray');
        result += this.#icon(
          this.websites[this.domains[i]].status ?? null,
          i == this.selected,
        );
        result += this.#color(
        ...
        result += this.#color(' │', 'gray');
        result += this.#icon(
          this.services[service].status ?? null,
          i - 1 == this.selected,
        );
        result += this.#color(
  ...
  #spacing(text, len, direction) {
    if (direction == 'right')
      return ' '.repeat(len - this.#length(text)) + text;
    if (direction == 'center')
      return (
        ' '.repeat(Math.floor((len - this.#length(text)) / 2)) +
        text +
        ' '.repeat(Math.ceil((len - this.#length(text)) / 2))
      );
  ...
  async #status() {
    ...
    status.websites = Candy.config.websites
      ? Object.keys(Candy.config.websites).length
      : 0;
    ...
    if (global.trigger == 'candy') {
      if (args.length == 0) {
                ...
                case 'Status':
                  log(
                    title +
                      ...
                      ' : ' +
                      (status.online
                        ? '\x1b[32m ' + (await __('Online'))
                        : '\x1b[33m ' + (await __('Offline'))) +
                      '\x1b[0m',
                  );
                  break;
                case 'Uptime':
                ...
                case 'Auth':
                  log(
                    title +
                      ...
                      ' : ' +
                      (status.auth
                        ? '\x1b[32m ' + (await __('Logged in'))
                        : '\x1b[33m ' + (await __('Not logged in'))) +
                      '\x1b[0m',
                  );
                  break;
              }
        ...
        if (!status.auth)
          log(
            await __(
              'Login on %s to manage all your server operations.',
              '\x1b[95mhttps://candypack.dev\x1b[0m',
            ),
          );
        log();
  ...
  table(input) {
    ...
    for (const row of input) {
      for (const key of Object.keys(row)) {
        ...
        if (this.#length(row[key]) > width[key])
          width[key] = this.#length(row[key]);
      }
    }
    for (const row of input) {
      let insert = '';
      if (input.indexOf(row) == 0) {
        ...
        for (const key of Object.keys(row)) {
          result +=
            this.#color(this.#value(key), 'blue') +
            ' '.repeat(width[key] - this.#length(key)) +
            this.#color(' │ ', 'gray');
        }
        result += '\n';
      }
      let rowInsert = '├─';
      for (const key of Object.keys(row))
        rowInsert += '─'.repeat(width[key]) + '─┼─';
      rowInsert = rowInsert.substr(0, rowInsert.length - 3) + '─┤\n';
      rowInsert += '│ ';
      result += this.#color(rowInsert, 'gray');
      for (const key of Object.keys(row)) {
        result +=
          this.#value(row[key]) +
          ' '.repeat(width[key] - this.#length(row[key])) +
          this.#color(' │ ', 'gray');
      }
      result += '\n';
    }
    ...
    for (const key of Object.keys(input[0]))
      footer += '─'.repeat(width[key]) + '─┴─';
    footer = footer.substr(0, footer.length - 3) + '─┘';
    result += this.#color(footer, 'gray');
    console.log(result);
  }
