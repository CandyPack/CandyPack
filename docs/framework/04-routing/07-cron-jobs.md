...
module.exports = () => {
  const backupDir = path.join(__dir, 'backups')
...
// Usage - Every night at 02:00
Candy.Route.cron('backup').hour(2).minute(0)
```

### Log Cleanup with Inline Function
...
}).weekDay(0).hour(3).minute(0) // Every Sunday at 03:00
```
...
Candy.Route.cron(async () => {
  try {
    console.log('API synchronization started')
    ...
    // Save to database
    // await Candy.db.query('INSERT INTO sync_data ...', data)
    
    console.log('Synchronization completed')
  } catch (error) {
    console.error('Synchronization error:', error)
  }
}).everyMinute(15) // Every 15 minutes
```

### Mixed Usage Examples

```javascript
// Controller-based job
Candy.Route.cron('email-reports').everyDay(1).hour(8).minute(0)
...
Candy.Route.cron(() => {
  const now = new Date()
  console.log(`Weekly report generated at ${now}`)
}).weekDay(1).hour(9).minute(0) // Every Monday at 9 AM
```
