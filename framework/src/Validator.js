      ...
      const validateRequest = async () => {
        for (const method of Object.keys(this.#checklist)) {
          for (const key of Object.keys(this.#checklist[method])) {
                  ...
                  switch (inverse ? vars[0].substr(1) : vars[0]) {
                    ...
                    case 'numeric':
                      error =
                        value &&
                        value !== '' &&
                        !Candy.Var(value).is('numeric');
                      break;
                    case 'alpha':
                      error =
                        value && value !== '' && !Candy.Var(value).is('alpha');
                      break;
                    case 'alphaspace':
                      error =
                        value &&
                        value !== '' &&
                        !Candy.Var(value).is('alphaspace');
                      break;
                    case 'alphanumeric':
                      error =
                        value &&
                        value !== '' &&
                        !Candy.Var(value).is('alphanumeric');
                      break;
                    case 'alphanumericspace':
                      error =
                        value &&
                        value !== '' &&
                        !Candy.Var(value).is('alphanumericspace');
                      break;
                    case 'email':
                      error =
                        value && value !== '' && !Candy.Var(value).is('email');
                      break;
                    case 'ip':
                      error =
                        value && value !== '' && !Candy.Var(value).is('ip');
                      break;
                    case 'float':
                      error =
                        value && value !== '' && !Candy.Var(value).is('float');
                      break;
                    case 'mac':
                      error =
                        value && value !== '' && !Candy.Var(value).is('mac');
                      break;
                    case 'domain':
                      error =
                        value && value !== '' && !Candy.Var(value).is('domain');
                      break;
                    case 'url':
                      error =
                        value && value !== '' && !Candy.Var(value).is('url');
                      break;
                    case 'username':
                    ...
                    case 'date':
                      error =
                        ...
                        (strtotime(value) === false ||
                          !(strtotime(value) > strtotime(0)));
                      break;
                    case 'min':
                      error =
                        value && value !== '' && vars[1] && value < vars[1];
                      break;
                    case 'max':
                      error =
                        value && value !== '' && vars[1] && value > vars[1];
                    ...
                    case 'mindate':
                      error =
                        ...
                        value !== '' &&
                        vars[1] &&
                        strtotime(value) < strtotime(vars[1]);
                      break;
                    case 'maxdate':
                      error =
                        value &&
                    ...
                    case 'notin':
                      error =
                        value && vars[1] && strpos(value, vars[1]) !== false;
                      break;
                    case 'in':
                      error =
                        value && vars[1] && !(strpos(value, vars[1]) !== false);
                    ...
                    case 'user': {
                      ...
                      if (Candy.string(user_data).is('bcrypt'))
                        error =
                          value &&
                          (!Auth.check() || !Candy.hash(value, user_data));
                      else if (Candy.string(user_data).is('md5'))
                        error =
                          value && (!Auth.check() || !md5(value) === user_data);
                      else
                        error =
                          value &&
                          (!Auth.check() || value !== Auth.user(vars[1]));
                      break;
                    }
                  }
            ...
            if (error)
              this.#message[key] = this.#checklist[method][key].message;
          }
        }
        this.#completed = true;
        return resolve();
