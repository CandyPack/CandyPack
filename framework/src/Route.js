...
class Route {
    ...
    return new Promise((resolve) => {
      const checkUrl = async () => {
        ...
        if (
          ...
          Candy.Request.header('Referer').startsWith(
            (Candy.Request.ssl ? 'https://' : 'http://') +
              Candy.Request.host +
              '/',
          ) &&
          Candy.Request.header('X-Candy-Client') ==
            Candy.Request.cookie('candy_client')
        ) {
          Candy.Request.header(
            'Access-Control-Allow-Origin',
            (Candy.Request.ssl ? 'https://' : 'http://') + Candy.Request.host,
          );
          Candy.Request.header(
            'Cache-Control',
          ...
          return resolve({
            token: Candy.token(),
            page:
              this.routes[Candy.Request.route]['page'][url].file ||
              this.routes[Candy.Request.route].error[404].file ||
              '',
          });
        }
        if (Candy.Config.route && Candy.Config.route[url]) {
          Candy.Config.route[url] = Candy.Config.route[url].replace(
            '${candy}',
            `${__dir}/node_modules/candypack`,
          );
        ...
        for (let method of ['#' + Candy.Request.method, Candy.Request.method]) {
          ...
          if (controller) {
            if (
              !Candy.Request.method.startsWith('#') ||
              (await Candy.Auth.check())
            ) {
              Candy.Request.header(
                'Cache-Control',
                'no-store, no-cache, must-revalidate, max-age=0',
              ...
              if (
                ...
                (!(await Candy.request('_token')) ||
                  !Candy.token(await Candy.Request.request('_token')))
              )
                return resolve(Candy.Request.abort(401));
        ...
        if (
          ...
          typeof this.routes[Candy.Request.route]['#page'][url].cache ===
            'function'
        ) {
          if (await Candy.Auth.check()) {
            ...
            Candy.cookie(
              'candy_data',
              { page: Candy.Request.page, token: Candy.token() },
              { expires: null, httpOnly: false },
            );
            return resolve(
              this.routes[Candy.Request.route]['#page'][url].cache(Candy),
            );
          }
        }
        if (
        ...
        if (
          url &&
          !url.includes('/../') &&
          fs.existsSync(`${__dir}/public${url}`)
        ) {
          ...
          if (stat.isFile()) {
            ...
            Candy.Request.header('Cache-Control', 'public, max-age=31536000');
            Candy.Request.header('Content-Length', stat.size);
            return resolve(fs.readFileSync(`${__dir}/public${url}`));
          }
        }
  ...
  #init() {
    ...
    for (const file of fs.readdirSync(`${__dir}/controller/`)) {
      ...
      if (Candy.Route.class[name]) {
        if (
          Candy.Route.class[name].mtime >=
          fs.statSync(Candy.Route.class[name].path).mtimeMs
        )
          continue;
        delete global[name];
    ...
    for (const file of dir) {
      ...
      for (const type of [
        ...
        for (const route in this.routes[Candy.Route.buff][type]) {
          if (
            routes2[Candy.Route.buff] >
            this.routes[Candy.Route.buff][type][route].loaded
          ) {
            delete require.cache[
              require.resolve(this.routes[Candy.Route.buff][type][route].path)
            ];
            delete this.routes[Candy.Route.buff][type][route];
          } else if (this.routes[Candy.Route.buff][type][route]) {
            if (
              typeof this.routes[Candy.Route.buff][type][route].type ===
              'function'
            )
              continue;
            if (
              this.routes[Candy.Route.buff][type][route].mtime <
              fs.statSync(this.routes[Candy.Route.buff][type][route].path)
                .mtimeMs
            ) {
              delete require.cache[
                require.resolve(this.routes[Candy.Route.buff][type][route].path)
              ];
              this.routes[Candy.Route.buff][type][route].cache = require(
                this.routes[Candy.Route.buff][type][route].path,
              );
              this.routes[Candy.Route.buff][type][route].mtime = fs.statSync(
                this.routes[Candy.Route.buff][type][route].path,
              ).mtimeMs;
            }
          }
  ...
  set(type, url, file, options = {}) {
    ...
    if (!this.routes[Candy.Route.buff][type])
      this.routes[Candy.Route.buff][type] = {};
    if (this.routes[Candy.Route.buff][type][url]) {
      this.routes[Candy.Route.buff][type][url].loaded =
        routes2[Candy.Route.buff];
      if (
        this.routes[Candy.Route.buff][type][url].mtime <
    ...
    if (fs.existsSync(path)) {
      if (!this.routes[Candy.Route.buff][type][url])
        this.routes[Candy.Route.buff][type][url] = {};
      this.routes[Candy.Route.buff][type][url].cache =
        typeof file === 'function' ? file : require(path);
      this.routes[Candy.Route.buff][type][url].type =
        typeof file === 'function' ? 'function' : 'controller';
