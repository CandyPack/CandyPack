...
class Mysql {
  ...
  #define(table) {
    return new Promise((resolve) => {
      ...
      let columns = [];
      this.#conn.query(
        `SHOW COLUMNS FROM ${this.escape(table, 'table')}`,
        (err, result) => {
          if (err) {
            this.#error(err);
            this.#defining = false;
  ...
  get(b) {
    return new Promise((resolve) => {
      ...
      this.run(query).then(async (sql) => {
        //   console.log(sql);
        if (sql === false) return resolve(this.#error());
        for (let row of sql) {
          for (let [key, value] of Object.entries(row))
            row[key] = await this.type(key, value);
          data.push(row);
        }
  ...
  insert(arr) {
    return new Promise((resolve) => {
      this.id = 1;
      this.#valuesExtract(arr).then((ext) => {
  ...
  order(v1, v2 = 'asc') {
    ...
    /* }else */ this.#arr['order by'] =
      this.escape(v1, 'col') + (v2.toLowerCase() == 'desc' ? ' DESC' : ' ASC');
    return this;
  }

  orWhere(...args) {
    this.#arr.where =
      this.#arr.where && this.#arr.where.trim() != ''
        ? `${this.#arr.where} OR ${this.#whereExtract(args)}`
        : this.#whereExtract(args);
    return this;
  }
  ...
  select(...args) {
    ...
    if (
      args.length == 1 &&
      (typeof args[0] === 'object' || args[0] instanceof Raw)
    ) {
      if (args[0] instanceof Raw) {
        ...
        for (let key of Object.keys(args[0])) {
          ...
          if (isNaN(key))
            this.#arr['select'].push(
              this.escape(key, 'col') + ' AS ' + this.escape(value, 'col'),
            );
      ...
      const setVars = async () => {
        ...
        if (!['array', 'object'].includes(typeof arr) && val !== undefined)
          vars +=
            this.escape(arr, 'col') +
            ' = ' +
            this.escape(await this.type(arr, val, 'encode')) +
            ',';
        else
          for (let [key, value] of Object.entries(arr))
            vars +=
              this.escape(key, 'col') +
              ' = ' +
              this.escape(await this.type(key, value, 'encode')) +
              ',';
  ...
  groupBy(...args) {
    ...
    //           }
    //         }
    /*       } else */ for (let key of args)
      select.push(this.escape(key, 'col'));
      ...
      if (this.#arr[key]) {
        if (Array.isArray(this.#arr[key])) {
          query +=
            ...
            key.toUpperCase() +
            ' ' +
            this.#arr[key].join(' ' + key.toUpperCase() + ' ');
        } else {
  ...
  type(col, value, action = 'decode') {
    return new Promise((resolve) => {
      const getType = async () => {
        ...
        if (!this.types[col]) {
          ...
          for (const key of Object.keys(this.#table)) {
            ...
            if (!this.#arr.select && this.#table[key].columns[col]?.Type) {
              this.types[col] =
                this.#table[key].columns[col]?.Type ?? this.types[col];
              break;
            } else if (!this.#arr.select) {
              continue;
            } else if (
              Candy.Var(this.#arr.select).contains(' AS "' + col + '"')
            ) {
              ...
              Candy.Var(this.#arr.select).containsAny(
                ' `' + col + '`',
                ' `' + key + '`.`' + col + '`',
              )
            ) {
        ...
        if (action == 'decode') {
          if (Candy.Var(this.types[col]).isBegin('tinyint(1)'))
            value = value ? true : false;
          else if (Candy.Var(this.types[col]).isBegin('int'))
            value = parseInt(value);
          else if (Candy.Var(this.types[col]).isBegin('double'))
            value = parseFloat(value);
          else if (Candy.Var(this.types[col]).isBegin('float'))
            value = parseFloat(value);
          else if (Candy.Var(this.types[col]).isBegin('boolean'))
            value = parseInt(value);
          else if (Candy.Var(this.types[col]).isBegin('json'))
          ...
          if (Candy.Var(this.types[col]).isBegin('tinyint(1)'))
            ...
            Candy.Var(this.types[col]).isBegin('date', 'datetime', 'timestamp')
          )
            value = Candy.Var(value).date('Y-m-d H:i:s');
        }
  ...
  where(...args) {
    if (
      ...
      this.#arr.where = this.#whereExtract([
        this.#table[this.#arr.table].primary,
        args[0],
      ]);
    } else if (args.length > 0) {
      this.#arr.where =
        this.#arr.where && this.#arr.where.trim() != ''
          ? `${this.#arr.where} AND ${this.#whereExtract(args)}`
          : this.#whereExtract(args);
    }
    return this;
  }
  ...
  #whereExtract(arr) {
    ...
    for (const key of arr) {
      if (
        key &&
        ['array', 'object'].includes(typeof key) &&
        state != 'IN' &&
        state != 'NOT IN' &&
        !(key instanceof Raw)
      ) {
        q +=
          last == 1
            ? ' AND ' + this.#whereExtract(key)
            : this.#whereExtract(key);
        in_arr = true;
        last = 1;
        ...
        state = this.#statements.includes(key.toUpperCase())
          ? key.toUpperCase()
          : '=';
        q += ' ' + state;
        last = 1;
...
module.exports = {
  ...
  init: function () {
    return new Promise((resolve) => {
      ...
      const initDb = async () => {
        let multiple =
          typeof Candy.Config.database[
            Object.keys(Candy.Config.database)[0]
          ] === 'object';
        let dbs = multiple
          ? Candy.Config.database
          : { default: Candy.Config.database };
        for (let key of Object.keys(dbs)) {
          ...
          await Candy.Mysql.conn[key].connect();
