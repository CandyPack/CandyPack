    ...
    if (args.length == 1 && (typeof args[0] === 'object' || args[0] instanceof Raw)) {
      if (args[0] instanceof Raw) {
        ...
        for (let key of Object.keys(args[0])) {
          ...
          if (isNaN(key))
            this.#arr['select'].push(this.escape(key, 'col') + ' AS ' + this.escape(value, 'col'))
  ...
  async set(arr, val) {
    return new Promise(async (resolve, reject) => {
      ...
      if (!['array', 'object'].includes(typeof arr) && val !== undefined)
        vars +=
          this.escape(arr, 'col') + ' = ' + this.escape(await this.type(arr, val, 'encode')) + ','
      else
    ...
    for (let key of arr_q) {
      if (this.#arr[key]) {
        if (Array.isArray(this.#arr[key])) {
          query +=
            ' ' + key.toUpperCase() + ' ' + this.#arr[key].join(' ' + key.toUpperCase() + ' ')
        } else {
  ...
  async type(col, value, action = 'decode') {
    return new Promise(async (resolve, reject) => {
      ...
      if (action == 'decode') {
        ...
        else if (Candy.Var(this.types[col]).isBegin('date', 'datetime', 'timestamp'))
...
module.exports = {
  ...
  init: function () {
    return new Promise(async resolve => {
      ...
      let multiple =
        typeof Candy.Config.database[Object.keys(Candy.Config.database)[0]] === 'object'
