    ...
    #render(file) {
        ...
        if(this.#cache[file]?.mtime !== mtime){
            ...
            for(let key in this.#functions){
                ...
                let matches = func.close ? result.match(new RegExp(`${key}[\\s\\S]*?${func.close}`, 'g'))
                ...
                for(let match of matches){
                    ...
                    let vars = {};
                    if(func.arguments) for(let arg of args){
                    ...
                    if(func.close){
                        ...
                        result = result.replace(`<candy-${match}>`, 
                                                (func.replace ? `<${[func.replace,att].join(' ')}>` : '') + '`; ' + fun + ' html += `');
                        result = result.replace(`</candy-${key}>`, '`; ' + (func.end ?? '}') + ' html += `' + (func.replace ? `</${func.replace}>` : ''));
                    }
                }
            }
