...
class View {
  ...
  print() {
    ...
    if (
      this.#part.skeleton &&
      fs.existsSync(`./skeleton/${this.#part.skeleton}.html`)
    ) {
      result = fs.readFileSync(
        `./skeleton/${this.#part.skeleton}.html`,
        'utf8',
      );
      for (let key in this.#part) {
        if (['all', 'skeleton'].includes(key)) continue;
        if (!this.#part[key]) continue;
        if (this.#part[key].includes('.'))
          this.#part[key] = this.#part[key].replace(/\./g, '/');
        if (fs.existsSync(`./view/${key}/${this.#part[key]}.html`)) {
          result = result.replace(
            `{{ ${key.toUpperCase()} }}`,
            this.#render(`./view/${key}/${this.#part[key]}.html`),
          );
        }
      }
      if (this.#part.all) {
  ...
  #render(file) {
    ...
    if (this.#cache[file]?.mtime !== mtime) {
        ...
        for (let match of matches) {
          ...
          for (let key in func.arguments) {
            if (vars[key] === undefined) {
              if (func.arguments[key] === null)
                console.error(
                  `"${key}" is required for "${match}"\n  in "${file}"`,
                );
          ...
          if (func.close) {
            result = result.replace(
              match,
              fun +
                match.substring(key.length, match.length - func.close.length) +
                func.end,
            );
          } else {
            result = result.replace(
              `<candy-${match}>`,
              (func.replace ? `<${[func.replace, att].join(' ')}>` : '') +
                '`; ' +
                fun +
                ' html += `',
            );
      ...
      if (!fs.existsSync('./storage/cache'))
