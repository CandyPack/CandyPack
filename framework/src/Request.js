...
class Request {
  ...
  constructor(id, req, res, candy) {
    ...
    this.ip = (
      this.header('x-candy-connection-remoteaddress') ??
    ...
    this.#timeout = setTimeout(
      () => !this.res.finished && this.abort(408),
      Candy.Config.request.timeout,
    );
    this.#data();
    if (!Candy.Request) Candy.Request = {};
    if (
      !this.cookie('candy_client') ||
      !this.session('_client') ||
      this.session('_client') !== this.cookie('candy_client')
    ) {
      let client = crypto
        .createHash('md5')
        .update(
          this.ip + this.id + Date.now().toString() + Math.random().toString(),
        )
        .digest('hex');
  ...
  async abort(code) {
    this.status(code);
    let result =
      { 401: 'Unauthorized', 404: 'Not Found', 408: 'Request Timeout' }[code] ??
      null;
    if (
      Candy.Route.routes[this.route].error &&
      Candy.Route.routes[this.route].error[code]
    )
      result = await Candy.Route.routes[this.route].error[code].cache(
        this.#candy,
      );
    this.end(result);
  }
  ...
  cookie(key, value, options = {}) {
    ...
    for (const option of Object.keys(options))
      if (options[option]) cookie += `; ${option}=${options[option]}`;
  ...
  #data() {
    ...
    this.req.on('data', (chunk) => {
      ...
      if (body.length > 1e6) {
        ...
        if (body.length > 0 && body.indexOf('Content-Disposition') === -1)
          return;
  ...
  header(key, value) {
    if (value === null) delete this.#headers[key];
      ...
      for (const header of Object.keys(this.req.headers))
  ...
  on(event, callback) {
    if (this.#event[event])
      this.#event[event].push({ callback: callback, active: false });
    else return false;
  }
  ...
  async request(key, method) {
    ...
    if ((!method || method === 'post') && (this.data.post[key] ?? null))
      return this.data.post[key];
    if ((!method || method === 'get') && (this.data.get[key] ?? null))
      return this.data.get[key];
    if ((!method || method === 'url') && (this.data.url[key] ?? null))
      return this.data.url[key];
    return new Promise((resolve, reject) => {
      let interval = setInterval(() => {
        if (
          this.data.post[key] !== undefined ||
          this.data.get[key] !== undefined ||
          this.#complete
        ) {
          clearInterval(interval);
          if (!key && !method) resolve(this.data.post);
          else if (!key && method) resolve(this.data[method.toLowerCase()]);
          else if (this.data.post[key] !== undefined && method !== 'GET')
            resolve(this.data.post[key]);
          else if (this.data.get[key] !== undefined && method !== 'GET')
  ...
  session(key, value) {
    ...
    if (!Candy.Request.session[pub + '-' + pri])
      Candy.Request.session[pub + '-' + pri] = {};
    if (value === undefined)
      return Candy.Request.session[pub + '-' + pri][key] ?? null;
  ...
  set(key, value, ajax = false) {
    if (typeof key === 'object')
      for (const k in key) this.variables[k] = { value: key[k], ajax: ajax };
