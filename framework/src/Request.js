...
class Request {
  ...
  constructor(id, req, res, candy) {
    ...
    this.ip = (
      this.header('x-candy-connection-remoteaddress') ?? req.connection.remoteAddress
    ).replace('::ffff:', '')
  ...
  cookie(key, value, options = {}) {
    ...
    if (options.expires === undefined)
      options.expires = new Date(Date.now() + 1000 * 60 * 60 * 24 * 365).toUTCString()
  ...
  header(key, value) {
    if (value === null) delete this.#headers[key]
      ...
      for (const header of Object.keys(this.req.headers))
        if (header.toLowerCase() === key.toLowerCase()) return this.req.headers[header]
  }
