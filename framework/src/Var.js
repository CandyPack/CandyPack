...
class Var {
  ...
  is(...args) {
    ...
    if (args.includes('alpha'))
      result = (result || any) && ((any && result) || /^[A-Za-z]+$/.test(this.#value))
    if (args.includes('alphaspace'))
      result = (result || any) && ((any && result) || /^[A-Za-z\s]+$/.test(this.#value))
    if (args.includes('alphanumeric'))
      result = (result || any) && ((any && result) || /^[A-Za-z0-9]+$/.test(this.#value))
    if (args.includes('alphanumericspace'))
      result = (result || any) && ((any && result) || /^[A-Za-z0-9\s]+$/.test(this.#value))
    if (args.includes('bcrypt'))
      result = (result || any) && ((any && result) || /^\$2[ayb]\$.{56}$/.test(this.#value))
    if (args.includes('date'))
      result = (result || any) && ((any && result) || !isNaN(Date.parse(this.#value)))
    if (args.includes('domain'))
      result =
        (result || any) &&
        ((any && result) || /^([a-z0-9\-]+\.){1,2}[a-z]{2,6}$/i.test(this.#value))
    if (args.includes('email'))
      result =
        (result || any) &&
        ((any && result) || /^[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,6}$/i.test(this.#value))
    if (args.includes('float'))
      result = (result || any) && ((any && result) || !isNaN(parseFloat(this.#value)))
    if (args.includes('host'))
      result =
        (result || any) && ((any && result) || /^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$/.test(this.#value))
    if (args.includes('ip'))
      result =
        (result || any) && ((any && result) || /^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$/.test(this.#value))
    if (args.includes('json'))
      result =
        (result || any) &&
        ((any && result) || (JSON.parse(this.#value) && JSON.parse(this.#value).length >= 0))
    if (args.includes('mac'))
      result =
        (result || any) &&
        ((any && result) || /^([0-9A-Fa-f]{2}[:-]){5}([0-9A-Fa-f]{2})$/.test(this.#value))
    if (args.includes('md5'))
      result = (result || any) && ((any && result) || /^[a-f0-9A-F]{32}$/.test(this.#value))
    if (args.includes('numeric'))
      result = (result || any) && ((any && result) || !isNaN(this.#value))
    if (args.includes('url'))
      result =
        (result || any) &&
        ((any && result) || /^[a-z0-9]+:\/\/[a-z0-9]+\.[a-z]{2,6}/i.test(this.#value))
    ...
    if (args.includes('xss'))
      result =
        (result || any) && ((any && result) || this.#value == this.#value.replace(/<[^>]*>/g, ''))
    return result
  }
  ...
  replace(...args) {
    ...
    if (['array', 'object'].includes(typeof this.#value)) {
      ...
      for (const key of Object.keys(this.#value))
        new_value[key] = Candy.Var(this.#value[key]).replace(args)
      return new_value
    }
