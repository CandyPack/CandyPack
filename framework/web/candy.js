class candy {
  ...
  action(obj) {
    if (obj.function)
      for (let func in obj.function) this.fn[func] = obj.function[func];
    if (obj.start)
      $(function () {
        obj.start();
      });
    ...
    if (obj.interval) {
      ...
      for (let interval in obj.interval) {
        ...
        if (
          obj.interval[interval].page &&
          obj.interval[interval].page != this.page()
        )
          continue;
        this.actions.interval[interval]._ = setInterval(
          obj.interval[interval].function,
          obj.interval[interval].interval ?? 1000,
        );
      }
    }
    for (let key in obj) {
      if (['function', 'start', 'load', 'page', 'interval'].includes(key))
        continue;
  ...
  data() {
    ...
    return JSON.parse(
      unescape(document.cookie.split('candy_data=')[1].split(';')[0]),
    );
  }
  ...
  form(obj, callback) {
    ...
    $(document).on('submit.candy', obj.form, function (e) {
      ...
      actions.candy &&
        ...
        form
          .find(
            `select.${actions.candy.form.input.class.invalid},input.${actions.candy.form.input.class.invalid},textarea.${actions.candy.form.input.class.invalid}`,
          )
          .removeClass(actions.candy.form.input.class.invalid);
      if (obj.messages !== false) {
        if (
          obj.messages == undefined ||
          obj.messages == true ||
          obj.messages.includes('error')
        )
          form.find('*[candy-form-error]').hide();
        if (
          obj.messages == undefined ||
          obj.messages == true ||
          obj.messages.includes('success')
        )
          form.find('*[candy-form-success]').hide();
      }
      if (form.find('input[type=file]').length > 0) {
        ...
        datastring = form.serialize() + '&_token=' + Candy.token();
        cache = true;
        ...
        success: function (data) {
          ...
          if (obj.messages == undefined || obj.messages) {
            if (
              ...
              if (form.find('*[candy-form-success]').length)
                form
                  .find('*[candy-form-success]')
                  .html(data.result.message)
                  .fadeIn();
              else
                form.append(
                  `<span candy-form-success="${obj.form}">${data.result.message}</span>`,
                );
            } else {
              ...
              if (actions.candy && actions.candy.form) {
                if (actions.candy.form.input) {
                  if (actions.candy.form.input.class) {
                    if (actions.candy.form.input.class.invalid)
                      invalid_input_class =
                        actions.candy.form.input.class.invalid;
                  }
                  if (actions.candy.form.input.style) {
                    if (actions.candy.form.input.style.invalid)
                      invalid_input_style =
                        actions.candy.form.input.style.invalid;
                  }
                }
                if (actions.candy.form.span) {
                  if (actions.candy.form.span.class) {
                    if (actions.candy.form.span.class.invalid)
                      invalid_span_class =
                        actions.candy.form.span.class.invalid;
                  }
                  if (actions.candy.form.span.style) {
                    if (actions.candy.form.span.style.invalid)
                      invalid_span_style =
                        actions.candy.form.span.style.invalid;
                  }
                }
              }
              $.each(data.errors, function (name, message) {
                if (message) {
                  if (form.find(`[candy-form-error="${name}"]`).length)
                    form
                      .find(`[candy-form-error="${name}"]`)
                      .html(message)
                      .fadeIn();
                  else
                    form
                ...
                form
                  .find('*[name="' + name + '"]')
                  .addClass(invalid_input_class);
                form
                  .find('*[name="' + name + '"]')
                  .on('focus.candy', function () {
