...
class Table {
   ...
   constructor(table, conn){
    ...
    run(type = null){
      ...
      if(type == "insert")      query = `INSERT ${this.arr.ignore ? 'IGNORE' : ''} INTO ${this.escape(this.arr.table,'table')} ${this.arr.into} VALUES ${this.arr.values}`;
      if(type == 'get')         query = `SELECT ${this.arr.select ? this.arr.select : '*'} FROM ${this.escape(this.arr.table,'table')} ${query}`;
...
//     }
    whereExtract(arr){
      ...
      for (const key of arr) {
        if(Array.isArray(key) && (state != 'IN' && state != 'NOT IN') && !typeof key.raw !== 'function'){
          ...
          // if(!Array(key).isArray() && this.val_statements.includes(key.toUpperCase())){
          //   q += " " + key.toUpperCase();
          // }else{
            q += " =" + this.escape(key);
          // }
    ...
    escape(v, type){
      if(!type) type = 'value';
      if(v.raw && typeof v.raw == 'function') return ' ' + v.raw() + ' ';
      if(type == 'value'){
...
module.exports = {
    conn: {},
    init: function(){
      if(Candy._config.mysql){
          let db = Candy._config.mysql;
