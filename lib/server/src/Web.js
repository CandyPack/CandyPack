...
async function start(domain){
    ...
    if(!website) return;
    if(website.status == 'errored' && (Date.now() - website.updated < error_counts[domain] * 1000 || error_counts[domain] > 10)) return;
    let port = 60000;
    let using = false;
    do {
        if(ports[port]){
    ...
    var child = spawn('node', [website.path + '/index.js', port]);
    let pid = child.pid;
